...
function checkBundledProductCanBeAdded(childProducts, productLineItems, quantity) {
    ...
    childProducts.forEach(function (childProduct) {
        ...
        atsValueByChildPid[childProduct.pid] = Math.min(apiChildProduct.availabilityModel.inventoryRecord.ATS.value, apiChildProduct.custom.maxOrderableQty || 999);
    });
...
function addProductToCart(currentBasket, productId, quantity, childProducts, options) {
    ...
    var totalQtyRequested = 0;
    var canBeAdded = false;
    var maxOrderableQty = 0;
    if (product.bundle) {
        canBeAdded = checkBundledProductCanBeAdded(childProducts, productLineItems, quantity);
        var maxOrderableQtyArr = [999];
        childProducts.forEach(function (childProduct) {
            var apiChildProduct = ProductMgr.getProduct(childProduct.pid);
            var max = Math.min(apiChildProduct.availabilityModel.inventoryRecord.ATS.value, apiChildProduct.custom.maxOrderableQty || 999);
            maxOrderableQtyArr.push(max);
        });
        maxOrderableQty = Math.min.apply(null, maxOrderableQtyArr);
    } else {
        ...
        maxOrderableQty = product.custom.maxOrderableQty || 999;

        canBeAdded =
            (perpetual
            || totalQtyRequested <= maxOrderableQty);
    }
